...
def FINAL(Type,Duration,Budget,TYPE,Ques):
  ...
  time_slot = []  # Define time_slot as a global variable

  jaipur_poi_df = pd.read_csv('data/jaipur-poi.csv')
  dist_only_matrix_df = pd.read_csv('data/dist_only_matrix.csv')

  # Check if 'PID' column exists, if not, create it
  if 'PID' not in jaipur_poi_df.columns:
      jaipur_poi_df['PID'] = jaipur_poi_df.index.map(lambda x: f'POI{x+1}')

  if 'PID' not in dist_only_matrix_df.columns:
      dist_only_matrix_df['PID'] = dist_only_matrix_df.index.map(lambda x: f'POI{x+1}')
  ...
  timepoiplotlydf
  def get_endtime(pid):
    filtered_df = timepoiplotlydf[timepoiplotlydf['PID']==pid]['Finish']
    if filtered_df.empty:
        print(f"Warning: No end time found for PID {pid}")
        return None  # or some default value
    time1 = str(filtered_df.iloc[0])[11:16]
  ...
  def refresh_map():
    ...
    # Add this line to include a default attribution
    attribution = 'Map data © <a href="https://openstreetmap.org">OpenStreetMap</a> contributors'

    # If you're using custom tiles, add the attribution there:
    folium.TileLayer(
